...
const Firebase = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // first paint.
      const { analytics, logEvent, setUserProperties } = await loadFirebaseModule();
      ...
      try {
        ...
        // Ignore storage access issues (private mode / strict browser settings)
      }
      ...
      logEvent(analytics, 'landing_context', {
        path: window.location.pathname,
      ...
      logEvent(analytics, isFirstVisit ? 'first_time_user' : 'returning_user', {
        path: window.location.pathname,
      ...
      const trackSessionEnd = () => {
        ...
        const timeOnPageSec = Math.floor((Date.now() - sessionStartedAt) / 1000);
        logEvent(analytics, 'engagement_summary', {
          path: window.location.pathname,
      ...
      // Capture immediate state in case the user bounces quickly.
      handleScroll();
